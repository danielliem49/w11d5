...
import { getPokemonTypes, editPokemonDetails } from '../store/pokemon';
...
const EditPokemonForm = ({ pokemon, hideForm }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // the form DOES have an accurate payload of pokemon info
    console.log(payload)

    // this is trying to dispatch the edit action, so this payload gets saved to the db
    let updatedPokemon = await dispatch(editPokemonDetails(payload));
    if (updatedPokemon) {
      // just copied the below line
