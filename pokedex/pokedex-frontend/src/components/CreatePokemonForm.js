...
const CreatePokemonForm = ({ hideForm }) => {
  ...
  // the form at the bottom of this file will call this function when the button is clicked
  const handleSubmit = async (e) => {
    e.preventDefault();

    const payload = {number, attack, defense, imageUrl, name, type, move1, move2, moves: [move1, move2]};

    // trying to dispatch the createPokemon action with the payload pokemon info, which comes from the html inputs that the user types into
    // needs to await the result because dispatching is a request
